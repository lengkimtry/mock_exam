...
export class ExamController {
  ...
  @Post()
  async createExam(
    @Body() body: { name: string; subjectId: string; difficultyLevel: string },
  ) {
    // Normalize difficultyLevel to match enum values
    const normalizedDifficultyLevel =
      body.difficultyLevel.charAt(0).toUpperCase() +
      body.difficultyLevel.slice(1).toLowerCase();
  ...
  async updateExam(
    ...
    return this.examService.updateExam(id, {
      ...updateData,
      difficultyLevel: updateData.difficultyLevel as
        | DifficultyLevel
        | undefined,
    });
  }
  ...
  async filterExams(
    ...
    body: {
      universityId: string;
      subjectId: string;
      filterType?: 'year' | 'random';
    },
  ) {
    if (body.filterType === 'random') {
      return this.examService.getRandomExercisesByUniversityAndSubject(
        body.universityId,
        body.subjectId,
  ...
  @Get('subject/:subjectId/random')
  async getRandomExercisesForSubject(@Param('subjectId') subjectId: string) {
    return this.examService.getRandomExercises(subjectId);
  }

  @Get('university/:universityId/subject/:subjectId/random')
  async getRandomExercisesForExam(
